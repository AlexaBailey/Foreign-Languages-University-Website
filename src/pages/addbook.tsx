...
export default function AddBook() {
  ...
  const handleSubmit = (event) => {
    ...
    axios.post('http://localhost:8800/addbook', formData)
      .then((response) => {
...
<div style={{display:'flex', flexDirection:'column'}}>
  ...
  <div style={{display:'flex',justifyContent:'space-between', gap:40}}>
    ...
    <p>Автор</p>
    <input  name="writer" onChange={(e)=>setWriter(e.target.value)}  className='my-input' placeholder='ФИО автора'/>
    <p>Специальность</p>
    <input className='my-input'name="spec" onChange={(e)=>setSpec(e.target.value)}  placeholder='КПО, АИСД, ... '/>

    </div>
    <div style={{display:'flex', flexDirection:'column', gap:6}}>
    <p>Дата издания</p>
    <input className='my-input'name="date" onChange={(e)=>setDate(e.target.value)}  placeholder='22.05.2023'/>
    
    
    <p>Описание</p>
    <textarea className='my-input'name="comments" onChange={(e)=>setComments(e.target.value)}  placeholder='Комметарий к работе'/>
